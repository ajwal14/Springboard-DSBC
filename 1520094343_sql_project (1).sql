...
SELECT DISTINCT members.surname AS member, facs.name AS facility
FROM  `Members` members
JOIN  `Bookings` bks ON memberss.memid = bks.memid
...
SELECT members.surname AS member, facs.name AS facility,
CASE
WHEN members.memid =0
THEN bks.slots * facs.guestcost
ELSE bks.slots * facs.membercost
END AS cost
FROM  `Members` memberss
JOIN  `Bookings` bks ON memberss.memid = bks.memid
